...
const ApplicationDisplay: React.FC = () => {
  ...
  const [loadingTestResults, setLoadingTestResults] =
    ...
    /*
        const Panel = () => {
            {tests.map((test, index) => {
                return (
                    <div key={index} className='w-1/2 pt-20 '>
                        <div className='w-full flex justify-center'>
                            ...
                            <div className='w-1/2 flex justify-center'>
                                {test.Created_at.split('.')[0].split("T").join("  ")}
                            </div>
      ...
      <div className="w-full flex flex-wrap">
        {tests.map((test, index) => {
          return (
            <div key={index} className="w-1/2 pt-20 ">
              <div className="w-full flex justify-center">
                <div className="w-1/2 flex justify-center">Authenticated :</div>
                <div className="w-1/2 flex justify-center">
                  {test.Authentication.toString()}
                </div>
              </div>
              <div className="w-full flex justify-center">
                <div className="w-1/2 flex justify-center">Created At :</div>
                <div className="w-1/2 flex justify-center">
                  {test.Created_at.split('.')[0].split('T').join('  ')}
                </div>
              </div>
              <div className="w-full flex justify-center">
                <div className="w-1/2 flex justify-center">Endpoint :</div>
                <div className="w-1/2 flex justify-center">{test.Endpoint}</div>
              </div>
              <div className="w-full flex justify-center">
                <div className="w-1/2 flex justify-center">Authenticated :</div>
                <div className="w-1/2 flex justify-center">
              ...
              <div className="w-full flex justify-center">
                <div className="w-1/2 flex justify-center">Methods :</div>
                <div className="w-1/2 flex justify-center">{test.Methods}</div>
              </div>
              <div className="w-full flex justify-center">
                <div className="w-1/2 flex justify-center">Headers :</div>
                <div className="w-1/2 flex justify-center">{test.Headers}</div>
              </div>
            </div>
          );
        })}
  ...
  return (
    <div className="w-full flex flex-wrap justify-around">
      <div className="w-full flex justify-around p-30">
        <button
          className="bg-blue-500 w-40 h-12 rounded-md"
        ...
        <button
          className="bg-blue-500 w-40 h-12 rounded-md"
          onClick={() => navigator(`/results/${application.Id}`)}
        >
          View Results
        </button>
        <button className="bg-blue-500 w-40 h-12 rounded-md" onClick={runTests}>
          Run Tests
        </button>
      </div>
        ...
        <div className="w-1/2 flex flex-wrap justify-center p-5">
          ...
          <div className="w-full flex justify-center">
            <p className="text-2xl">
              {application?.Time_Created.split('.')[0].split('T').join('  ')}
            </p>
          </div>
          ...
          <div className="w-full flex justify-center">
            <p className="text-2xl">
              {application?.Time_Edited.split('.')[0].split('T').join('  ')}
            </p>
          </div>
        </div>
      </div>

      <div className="w-full flex justify-center p-5 flex-wrap">
        <div className="w-full flex justify-center text-center">
          <h1 className="text-3xl">Tests</h1>
        </div>

        <div className="w-full flex flex-wrap justify-center p-5 text-center">
          {TestsPanel()}
        </div>
      </div>
    </div>
